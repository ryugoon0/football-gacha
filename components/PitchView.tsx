  ...
  return (
    <div className="pitch relative aspect-[3/4] w-full overflow-hidden rounded-xl border border-white/15">
      ...
      {state.away.map((dot) => (
        <span
          key={dot.id}
          className="absolute h-4 w-4 -translate-x-1/2 translate-y-1/2 rounded-full border border-white/30 bg-slate-300/80 transition-all duration-200 ease-linear"
      ...
      <span
        className="absolute h-2.5 w-2.5 -translate-x-1/2 translate-y-1/2 rounded-full bg-white shadow-[0_0_8px_rgba(255,255,255,0.9)] transition-all duration-200 ease-linear"
        style={{ left: `${state.ball.x}%`, bottom: `${state.ball.y}%` }}
      />
