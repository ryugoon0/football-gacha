...
- `--upscale`: 원본이 작을 때(700px 아래) 4배 키운 뒤 자른다. GPU 가 있으면 큰
  사진에도 걸어 두는 편이 선명하다(1500px 원본이 40초쯤).
- `--head`: 머리 폭의 몇 배로 자를지(기본 2.8). 옆에 선 선수의 팔이 구석에
  남으면 2.3~2.5 로 줄인다. 작을수록 얼굴이 크게 잡힌다.
- `--no-flood`: 검은(또는 흰) 스튜디오 배경을 색으로 걷어내는 단계를 끈다. 검은 배경에
  검은 머리(레게 머리, 앞머리)가 닿아 있으면 그 단계가 머리를 같이 먹으므로 이 옵션을 준다.
- 이미 배경이 투명한 원본(구단 공식 컷아웃 등)은 분리를 건너뛰고 원본 알파를 그대로 쓴다.
  불투명으로 바꿔 다시 분리하면 머리 주변에 계단 모양 테가 생긴다.
...
- **원본 사진이 결과를 좌우한다.** 정면, 얼굴이 프레임의 1/3 이상, 손·타인과
  겹치지 않는 사진을 고른다. 옆얼굴이거나 손이 턱을 가리면 도구로는 못 고친다.
