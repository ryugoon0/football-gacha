...
export const PATCH_LOG: PatchEntry[] = [
  ...
  },
  {
    ...
    date: '2026-09-07',
    kind: 'balance',
    title: '월드 스카우트팩을 조각 600개로 · 프리미엄 스카우트에 월드 0.3% 복귀',
    detail: [
      '월드 스카우트팩을 조각 600개로 바로 열 수 있습니다(선물·월드 3장 합성과 함께 세 번째 경로). 골드로는 여전히 팔지 않습니다.',
      '프리미엄(리미티드) 스카우트에서 월드 카드가 0.3%로 다시 나옵니다. 일반 스카우트에서는 나오지 않습니다. 확률 안내가 함께 바뀌었습니다.',
    ],
  },
  {
  ...
  },
  {
    ...
    date: '2026-09-07',
    kind: 'feature',
    title: '리미티드 1차 「9월 1주 MOM」 20장 — 9월 8일(화) 14:00 ~ 14일(월) 09:00',
    detail: [
      '9월 4~6일 실제 경기의 활약 선수 20장이 리미티드 카드로 나옵니다. 같은 기간에는 클럽당 한 장이 기본 규칙이고, 정규 카드와 같은 이름·같은 선수이며 서로 한계 돌파 재료가 됩니다.',
      ...
      '9월 8일(화) 14:00 ~ 14일(월) 08:59에만 리미티드 스카우트(프리미엄 자리)의 리미티드 칸에서 나오고, 그 주 픽업도 이 카드들 중 하나입니다. 기간이 끝나면 풀에서 빠지고 받은 카드는 남습니다. 앨범에 「리미티드 · 9월 1주 MOM」 특별 세트가 생깁니다.',
    ],
  },
  {
    id: '2026-09-07-world-pack-limited-pack',
    date: '2026-09-07',
    kind: 'balance',
    title: '월드 스카우트 신설 — 월드 카드는 여기서만 · 리미티드 기간엔 프리미엄이 리미티드 스카우트로 · 조각으로 프리미엄 구매',
    detail: [
      '월드 스카우트팩이 생겼습니다. 팔지 않고 선물함이나 월드 카드 3장 합성(선수관리 → 승급 합성)으로만 생기며, 열면 플래티넘 90% · 월드 10%입니다. 월드 카드는 이제 이 팩에서만 나옵니다. 일반·프리미엄 스카우트와 조각 교환소의 월드 항목에서는 나오지 않습니다.',
      '리미티드 카드가 열려 있는 동안 프리미엄 스카우트가 리미티드 스카우트로 바뀝니다. 가격·10연속 보장·천장은 같고, 리미티드 칸이 7% 생기며 그만큼 일반·실버·골드가 비율대로 줄어듭니다(플래티넘 7%는 그대로). 리미티드 카드는 이 칸에서만 나옵니다. 첫 리미티드는 9월 8일(화) 14:00부터입니다.',
      '프리미엄(리미티드) 스카우트를 조각으로 열 수 있습니다. 1회 80조각, 10연속 720조각. 조각 주머니는 상점에서 내렸고 갖고 있던 것은 그대로 쓸 수 있습니다.',
      '확률 안내 페이지에 리미티드 스카우트·월드 스카우트 열이 추가됐습니다.',
    ],
  },
  {
    id: '2026-09-07-pick-tickets',
    date: '2026-09-07',
    kind: 'feature',
    title: '플래티넘·라이브 스카우트 지정권 — 원하는 카드 한 장을 골라 받는 아이템',
    detail: [
      '플래티넘 스카우트 지정권은 출시된 플래티넘 카드 중, 라이브 스카우트 지정권은 리미티드(라이브) 카드 중 원하는 선수를 골라 받습니다. 한 번 열렸던 주의 리미티드는 기간이 지나도 고를 수 있습니다. 아이템 탭 창고에서 「카드 고르기」로 씁니다.',
      '상점에서는 팔지 않고 선물·이벤트로만 줍니다. 받은 카드는 보관함이 가득 차도 들어옵니다.',
    ],
  },
  {
    id: '2026-09-07-tier-level-cap',
    date: '2026-09-07',
    kind: 'balance',
    title: '스쿼드 레벨 상한의 기준이 캐주얼 디비전에서 경쟁 리그 등급으로',
    detail: [
      '선발 11명 레벨 합 상한이 이제 경쟁 리그 등급을 따릅니다. 0등급 110 · 1등급 89 · 2등급 77 · 3등급 66이며, 아직 배정되지 않았으면 3등급 기준입니다. 캐주얼 모드·데일리 PvP·친선도 같은 상한을 씁니다.',
      '등급은 접속 때와 경쟁 리그 탭에서 서버와 맞춰지고, 승격·강등하면 다음 주 리그가 열릴 때 바뀝니다. 스쿼드 화면의 「라인업 등록 제한」에 기준 등급이 같이 보입니다.',
    ],
  },
  {
    id: '2026-09-07-regrade',
    date: '2026-09-07',
    kind: 'balance',
    title: '등급 재편 — 플래티넘 85·골드 79·실버 72, 레벨 한계 일반 7·실버 8·골드 9, 초과분은 골드로 보상',
    detail: [
      '등급 경계를 올렸습니다. 플래티넘은 OVR 85 이상이면서 클럽당 5장까지(넘치면 골드), 골드 79~84, 실버 72~78, 일반 71 이하. 플래티넘 187장→56장, 골드 685→366장, 실버 2,026→1,304장, 일반 678→1,850장. 코리아 리그·에레디비시·루소 프리메라에는 플래티넘이 없고, 하위 리그의 골드도 몇 장 없습니다. 리그 수준을 반영한 결과입니다.',
  ...
  {
    id: '2026-09-06-ulsan-release',
    date: '2026-09-06',
    kind: 'fix',
    title: '울산 호랑 스쿼드 33장 공개 — 파일럿 표시가 남아 뽑기·도감에서 빠져 있던 문제',
    detail: [
      '실제 스쿼드 명단을 처음 만들 때 시험용으로 넣은 울산 호랑이 계속 비공개로 남아 있었습니다. 그래서 울산 카드는 스카우트에 나오지 않았고, 옛 울산 카드를 정리할 때도 같은 클럽에 바꿔 줄 카드가 없어 코리아 리그 다른 클럽 카드로 옮겨졌습니다(퇴장감만 남은 이유입니다).',
      '이제 울산 호랑 33장이 스카우트·도감·앨범에 들어갑니다. 이미 다른 클럽으로 옮겨진 카드는 되돌리지 않습니다.',
    ],
  },
  {
    id: '2026-09-06-album-ever-owned',
    date: '2026-09-06',
    kind: 'balance',
    title: '앨범 — 한 번이라도 가진 카드면 등록, 팔거나 합성해도 기록이 남습니다',
    detail: [
      '앨범이 지금 손에 있는 카드가 아니라 보유 기록(도감)으로 완성 여부를 봅니다. 재료로 쓰거나 방출한 카드도 그대로 등록돼 있고, 서버 검증도 같은 기준입니다.',
    ],
  },
  {
    id: '2026-09-06-wide-midfielders',
    date: '2026-09-06',
    kind: 'balance',
    title: '측면 미드필더 48명 — 4-4-2·4-2-3-1의 LM·RM 자리를 채운다',
    detail: [
      '주 포지션이 LM·RM인 카드가 4장씩뿐이라 LM·RM 자리는 늘 −4를 안고 뛰었습니다. 실제로 측면 미드필더·윙백으로 뛰거나 그 성향이 강한 선수 48명(LM 22 · RM 26)의 주 포지션을 LM·RM으로 바꿨습니다. 원래 윙(LW·RW)은 부 포지션으로 남습니다.',
      'LM: 킨 루이스포토 · 마커스 태버니오 · 드와이트 맥닐런 · 이반 페리시츠 · 필리프 코스티츠 · 김승석 · 완델솔 · 문선믄 · 헤르만 발레란 · 후안미르 · 로제르 브루겐 · 키케 바르한 · 알바로 가르시안 · 루이스 리오한 · 토르강 아자른 · 이사크 얀손드 · 아르노 노르댕드 · 로베르트 스코운 · 마르코 그륄라 · 데리 셰르한튼 · 니클라스 베스텐',
      'RM: 이희근 · 소르바 토머슨 · 한지훈 · 고재헌 · 케이시 맥아티언 · 이청욘 · 제이컵 머핀 · 줄리아노 시메오나 · 테테 모렌토 · 루벤 가르시안 · 이시 팔라손드 · 로드리고 리켈멘 · 아이토르 루이발라 · 디에고 로페노 · 프셰미스와프 프란코프스킨 · 아론 되눔드 · 안톤 카덴 · 팀 스카르켄 · 안스가르 크나우픈 · 얀 틸마느 · 요나스 호프마느 · 루카스 바스케손 · 로빈 하큰 · 알레시오 제르비느 · 니코 곤살레노 · 알렉시스 살레마커르손 · 마테오 폴리타논',
      '이미 갖고 계신 카드는 LM·RM 자리에서 감점이 없어지고, LW·RW 자리에서는 −4가 붙습니다. 스쿼드에 그대로 둔 분은 자리를 한 번 확인해 주세요.',
    ],
  },
  {
    id: '2026-09-06-squad-care',
    date: '2026-09-06',
    kind: 'feature',
    title: '스쿼드에서도 치료·회복·잠금, 일괄 치료·회복 버튼',
    detail: [
      '스쿼드에서 선수를 눌러 상세를 열면 선수관리처럼 부상 치료·체력 회복·카드 잠금을 바로 할 수 있습니다.',
      '스쿼드 맨 위에 「부상 전원 치료」「체력 전원 회복」 버튼이 생겼습니다(선발·벤치 18명 대상, 인원과 총 비용 표시). 선수관리 관리 모드에는 보유 카드 전부를 대상으로 같은 버튼이 있습니다. 골드가 모자라면 닿는 데까지만 적용됩니다.',
    ],
  },
  {
    id: '2026-09-06-break-shortcut',
    date: '2026-09-06',
    kind: 'feature',
    title: '한계 돌파 — 돌파 가능한 카드가 맨 위에, 대상을 누르면 재료 자동 선택과 바로 옆 돌파 버튼',
    detail: [
      '선수관리 → 한계 돌파에서 여분 카드가 있어 지금 돌파할 수 있는 선수가 「돌파 가능」 표시로 맨 위에 옵니다. 대상을 누르면 여분 카드 한 장이 재료로 자동 선택되고, 카드 목록 바로 위에 돌파 버튼이 나옵니다. 돌파한 뒤 여분이 더 있으면 다음 재료가 이어서 잡힙니다.',
    ],
  },
  {
    id: '2026-09-06-friendly-to-minigames',
    date: '2026-09-06',
    kind: 'balance',
    title: '데일리 미니게임(친선) — 미니게임 탭으로 이동, 체력 소모 없음',
    detail: [
      '캐주얼 모드 옆에 있던 데일리 미니게임(순위와 무관한 친선 하루 10판)이 미니게임 탭 첫 칸으로 옮겨졌습니다. 골드와 경험치는 그대로 주고, 이제 체력을 쓰지 않습니다(부상도 없음). 하루 횟수와 친선 경기권은 그대로입니다.',
    ],
  },
  {
    id: '2026-09-06-bulk-fusion',
    date: '2026-09-06',
    kind: 'feature',
    title: '승급 합성 복귀 + 일괄 합성 — 잠그지 않은 카드를 등급별로 한꺼번에',
    detail: [
      '선수관리 → 승급 합성이 모두에게 다시 열렸습니다. 위쪽 「일괄 합성」에서 등급을 고르면 잠그지 않은 카드가 그 등급 장수(기본 3장)씩 묶여 한 단계 위 카드로 바뀝니다. 선발·벤치 18명과 잠근 카드는 건드리지 않고, 키운 카드(레벨 2 이상·경험치 있음)는 기본으로 제외합니다(체크 해제 가능).',
      '실행 전에 등급별 대상·횟수·결과·남는 장수·비용이 표로 보이고, 확인 창을 거칩니다. 만들어진 카드는 같은 실행에서 다시 합성하지 않습니다. 결과 카드는 한 단계 위 등급 출시 카드 중 균등 무작위입니다(확률 안내 페이지에 적었습니다).',
    ],
  },
  {
