...
export function migrationTarget(playerId: string): PlayerDef | null {
  ...
  const candidates = (targetsByClub.get(old.club) ?? []).filter((player) => POSITION_GROUP[player.position] === group)
  const pool = candidates.length > 0 ? candidates : targetsByClub.get(old.club) ?? []
  if (pool.length === 0) {
    mapping.set(playerId, '')
    return null
  }
  const rank = (player: PlayerDef) =>
    ...
    Math.abs(RARITIES.indexOf(player.rarity) - RARITIES.indexOf(old.rarity)) * 2
