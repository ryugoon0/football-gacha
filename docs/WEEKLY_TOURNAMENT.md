...
- 진짜 엔진 기반 정산(스냅샷 잠금 → 시뮬레이션 → 결과 기록)과 그 전제인
  "이 리그용 스쿼드 저장 화면" 자체.
- 징계(카드 누적) 시스템, 정식 주간 시스템(개막 배치 이후)의 신규 유저
  자동 배정, 일반 사용자용 화면(순위표·경기 결과 열람) — 전부 아직
  없습니다. 관리자 화면의 수동 생성 버튼(`WeeklyLeaguePanel`)은 이제
  자동 생성으로 대체돼 필요할 때만 쓰는 보조 수단으로 남아 있습니다.
