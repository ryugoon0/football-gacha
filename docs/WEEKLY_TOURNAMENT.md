...
이어받습니다** — 2주차(9/14 즈음)부터 필요한 "직전 주 순위로 승격·강등
반영" 로직은 이번 범위 밖입니다.
...
- 진짜 엔진 기반 정산(스냅샷 잠금 → 시뮬레이션 → 결과 기록)과 그 전제인
  "이 리그용 스쿼드 저장 화면" 자체.
- 2주차부터 필요한 승격·강등 반영, 8강 이후 컵 진행(이전 라운드가 끝나야
  다음 라운드가 생기는 방식은 설계돼 있지만 자동 트리거는 아직 없음),
  Masters Final 자동 생성, 징계(카드 누적) 시스템, 일반 사용자용 화면
  (순위표·경기 결과 열람) — 전부 아직 없습니다. 관리자 화면의 수동 생성
  버튼(`WeeklyLeaguePanel`)은 자동 생성으로 대체돼 필요할 때만 쓰는
  보조 수단으로 남아 있습니다.
