...
2026-09-04 새벽 세션에서 확정한 설계다. 아래 네 가지 결정은 같은 날 늦은
세션에서 사용자가 확정했다 — **구현 착수 승인됨.**
...
1. **예약 실행 방식 → 대안 B (트래픽 캐치업 + 5분 안전망 cron)**. Scheduled
   Edge Function(대안 A)이 이 플랜에서 되는지는 여전히 미확인이지만, 대안
   B로 확정했으므로 더 이상 막는 조건이 아니다.
2. **양방향 확장 범위 → 실유저-실유저, 실유저-AI 매치.** 최소 한쪽이 실유저인
   fixture는 진짜 스쿼드 시뮬레이션(`opponentSquad`)을 태운다. 양쪽 다 AI인
   fixture(아무도 안 보는 대다수)는 지금의 포아송 정산을 그대로 유지해
   서버 CPU 비용을 억제한다.
3. **작전카드 재화 → 기존 골드/조각 재사용.** 새 재화를 만들지 않는다.
   `lib/items.ts`에 `tacticCardPack` 아이템으로 골드나 조각으로 구매하고,
   인벤토리에 카드 종류별 개수로 보관한다.
4. **개막 배치 리그(placement-2026-09-04) → 지금 바로 소급 적용.** 이미
   포아송으로 정산된 경기와 새 엔진으로 정산될 경기가 같은 시즌 안에
   섞이는 형평성 트레이드오프를 사용자가 인지하고 승인했다.
...
- 개막 배치 리그에도 소급 적용하기로 했으므로(결정 4번), 이미 포아송으로
  정산된 배치 리그 경기와 새 엔진 배포 이후 정산될 경기가 같은 시즌 테이블에
  섞인다 — 새 엔진 배포 시점 이전/이후로 정산 방식이 갈린다는 것을 사용자가
  인지한 상태다. 배포 순서 10번(포아송 cron 교체)이 배치 리그에도 그대로
  적용된다.
