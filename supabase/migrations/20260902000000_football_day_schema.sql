-- 이 파일은 supabase/schema.sql의 사본입니다.
--
-- `supabase db push`는 아직 적용하지 않은 마이그레이션만 실행합니다. 이
-- 파일은 처음 한 번 전체를 올리기 위한 것이고, 앞으로 스키마가 바뀌면
-- 새 타임스탬프 파일이 하나씩 추가됩니다.
--
-- 안에 있는 모든 문장은 두 번 실행해도 안전합니다(if not exists /
-- drop ... if exists / create or replace). 손으로 이미 실행하신 부분이
-- 있어도 그대로 두고 지나갑니다.
